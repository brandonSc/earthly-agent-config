...
If there's demand from multiple users for this plugin, the cache database could be promoted to a hosted service — a lightweight SaaS that provides PURL → country-of-origin lookups. Instead of each customer maintaining their own cache, they'd query a shared, pre-warmed database.

This would look like:

```
Collector → Earthly Origin API (cloud) → cached result
                    │
                    └── Pre-warmed with millions of packages
                        Shared across all Lunar customers
                        99%+ cache hit rate
```
...
- **Verdict:** ⚠️ **Gets ~70% of the way.** Copyright lines are where country names most commonly appear (addresses), but misses "governing law" clauses and other in-body mentions. Also, coverage is inconsistent — some packages have zero attribution parties despite having license files.
...
A hybrid approach is possible: query ClearlyDefined as a first-pass enrichment (free, cached on their side), then fall back to local scanning for packages where their data is incomplete. The results from either source get cached in our Postgres table.
...
- ClearlyDefined enrichment as a secondary data source
