...
- The prototype already handles both — port with cleanup
...
- **ALL MUST be native-bash** — no `jq`. The prototype's Maven CI collector is very thorough with version detection (4 methods) — port the logic but rewrite without `jq`
...
- Write `"all"` or `"module"` scope
- Also write `.testing.source` to signal tests executed
...
- **Challenge:** Extracting coverage from XML in native bash without Python or jq
  - Option A: Use simple grep/awk to extract LINE counters from JaCoCo XML
  - Option B: If coverage extraction is too fragile in native bash, this specific sub-collector could use `base-main` image (exception to the native rule since XML parsing is genuinely complex)
- Write to `.lang.java.tests.coverage` AND `.testing.coverage`
...
4. **`test-coverage.sh` uses embedded Python** — may need to keep Python for XML parsing, but run in container (not native). Or use `base-main` as an exception.
5. **No `source` metadata** — add consistently
6. **Maven CI collector is very verbose with debug logging** — clean up `echo` debug lines, keep useful stderr messages
...
- `min-java-version`: Compare `.lang.java.version` against threshold. Note: Java versions are just major numbers (`"17"`, `"21"`) — use integer comparison, not semver
...
4. Implement the `java` policy (4 checks)
...
| Component | project | dependencies | min-java-version | min-maven/gradle-version |
|-----------|---------|-------------|-----------------|------------------------|
| hadoop (Java) | PASS (detects Java) | PASS (has pom.xml or gradle) | Verify version against 17 | Verify build tool version |
| spark (Java) | PASS (detects Java) | PASS | Verify version | Verify |
| backend (Go) | exits cleanly (no Java) | exits cleanly | SKIP (not Java) | SKIP (not Java) |
| frontend (Node) | exits cleanly (no Java) | exits cleanly | SKIP (not Java) | SKIP (not Java) |
...
3. **Gradle project without lockfile** — `dependencies.sh` exits cleanly (can't parse deps without lockfile)
